...
//    data.skeleton.anim[9][2].geometry.r = data.skeleton.anim[9][0].geometry.r;
    ...
    {
        if (rig_joint[k].size() != rig_weights[k].size())
        {
            std::cout << "Error in skinning_loader at k=" << k << " : rig_joint[k].size()=" << rig_joint[k].size() << " != rig_weights[k].size()=" << rig_weights[k].size() << std::endl;
            std::cout << rig_weights[k] << std::endl;
        }
...
data_loaded load_armadillo_data(GLuint shader)
{
    data_loaded data = load_basic_data_directory({ "assets/armadillo/", shader });
    return data;
}
