...
struct buffer
{
    ...
    /** Clear and resize the container (similar to vector.clear() followed by vector.resize())*/
    void resize_clear(size_t size);
    ...
    T const& operator[](size_t_debug index) const;
    T & operator[](size_t_debug index);
    T const& operator()(size_t index, std::experimental::source_location const& location_caller = std::experimental::source_location::current()) const;
    T & operator()(size_t index, std::experimental::source_location const& location_caller = std::experimental::source_location::current());
#else
 ...
 * \ingroup container
*/
template <typename T> std::string to_string(buffer<T> const& v, std::string const& separator=" ",std::string const& begin="", std::string const& end="");
...
template <typename T>
void buffer<T>::resize_clear(size_t size)
{
    clear();
    resize(size);
}
...
#if !defined(VCL_NO_DEBUG) && defined(SOURCE_LOCATION)
template <typename T>
T const& buffer<T>::operator[](size_t_debug index) const
{
    return (*this)(index.value, index.location_caller);
}
template <typename T>
T & buffer<T>::operator[](size_t_debug index)
{
    return (*this)(index.value, index.location_caller);
}
...
template <typename T>
T const& buffer<T>::operator()(size_t index, std::experimental::source_location const& location_caller) const
{
    assert_vcl(index<size(), buffer_access_error_message(index, size(), location_caller) );
    return data[index];
}

template <typename T>
T & buffer<T>::operator()(size_t index, std::experimental::source_location const& location_caller)
{
    assert_vcl(index<size(), buffer_access_error_message(index, size(), location_caller) );
    return data[index];
}
...
{
    assert_vcl(index < size(), "Wrong buffer index");
    return data[index];
}
template <typename T>
...
template <typename T> std::string to_string(buffer<T> const& v, std::string const& separator, std::string const& begin, std::string const& end)
{
    return vcl::detail::to_string_container(v, separator, begin, end);
}
