...
mesh mesh_primitive_triangle(vec3 const& p0, vec3 const& p1, vec3 const& p2,
                             vec4 const& c0, vec4 const& c1, vec4 const& c2)
{
    mesh shape;
    shape.position = {p0,p1,p2};
    shape.connectivity = {{0,1,2}};
    shape.normal = {normalize(cross(normalize(p1-p0),normalize(p2-p0)))};
    shape.color = {c0,c1,c2};
    shape.texture_uv = {{0,0}, {1,0}, {1,1}};

    return shape;
}

mesh mesh_primitive_pyramid(float L, float h)
{
    mesh shape;
    vec3 const p0 = {-L/2, -L/2, 0.0f};
    vec3 const p1 = { L/2, -L/2, 0.0f};
    vec3 const p2 = { L/2,  L/2, 0.0f};
    vec3 const p3 = {-L/2,  L/2, 0.0f};
    vec3 const pe = {  0 ,  0  ,  h  };

    shape.push_back(mesh_primitive_triangle(p0,p3,p2));
    shape.push_back(mesh_primitive_triangle(p0,p2,p1));

    shape.push_back(mesh_primitive_triangle(p0,p1,pe));
    shape.push_back(mesh_primitive_triangle(p1,p2,pe));
    shape.push_back(mesh_primitive_triangle(p2,p3,pe));
    shape.push_back(mesh_primitive_triangle(p3,p0,pe));
