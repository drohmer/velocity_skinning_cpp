...
void mesh_save_file_obj(std::string const& filename, mesh const& out)
{
    std::ofstream fid(filename, std::ofstream::out);
    assert_vcl_no_msg(fid.is_open());

    size_t const N_vertex = out.position.size();
    for(size_t k=0; k<N_vertex; ++k) {
        vec3 const& v = out.position[k];
        fid << "v "<<v[0]<<" "<<v[1]<<" "<<v[2]<<std::endl;
    }
    for(size_t k=0; k<N_vertex; ++k) {
        vec2 const& vt = out.texture_uv[k];
        fid << "vt "<<vt[0]<<" "<<vt[1]<<std::endl;
    }
    for(size_t k=0; k<N_vertex; ++k) {
        vec3 const& vn = out.normal[k];
        fid << "vn "<<vn[0]<<" "<<vn[1]<<" "<<vn[2]<<std::endl;
    }

    size_t const N_face = out.connectivity.size();
    for(size_t k=0; k<N_face; ++k)
    {
        uint3 const& f = out.connectivity[k];
        unsigned int const a = f[0]+1;
        unsigned int const b = f[1]+1;
        unsigned int const c = f[2]+1;
