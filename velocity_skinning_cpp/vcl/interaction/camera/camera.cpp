...
vec2 camera_scene::project_position(vec3 const& p) const
{
    mat4 const M = view_matrix();
    mat4 const P = perspective.matrix();

    vec4 const p_in  = {p.x, p.y, p.z, 1.0f};
    vec4 const p_out = P*M*p_in;
    vec3 const p_out_3d = vec3(p_out.x, p_out.y, p_out.z)/p_out.w;
    vec2 const p_out_2d= {p_out_3d.x, p_out_3d.y};
